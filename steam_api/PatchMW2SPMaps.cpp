...
//	{ "mp_nuked", "mp_terminal" },
	...
	{ "mp_cross_fire", "team_opforce_airborne" },
...
bool AssetRestrict_RestrictFromMaps(assetType_t type, const char* name, const char* zone)
{
	...
	if (type == ASSET_TYPE_FX)
	{
		...
		if (!strcmp(zone, "mp_nuked"))
		{
			if (!strstr(name, "double_rainbow") && !strstr(name, "smoke/car_damage_"))
			{
...
void GetBSPNameHookFunc(char* buffer, size_t size, const char* format, const char* mapname)
{
	...
	if (_strnicmp("mp_", mapname, 3) || !_stricmp(mapname, "mp_nuked") || !_stricmp(mapname, "mp_cross_fire") || !_stricmp(mapname, "mp_cargoship") || !_stricmp(mapname, "ui_viewer_mp")) // TODO: fix the hardcoded nuked reference
	{
		// SP
